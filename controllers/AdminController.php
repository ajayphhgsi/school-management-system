...
$this->render('admin/classes/edit', ['class' => $class, 'subjects' => $subjects, 'csrf_token' => $csrfToken]);
    ...
    public function marksEntry() {
        ...
        $classes = $this->db->select("SELECT * FROM classes $where ORDER BY class_name", $params);
        $csrfToken = $this->csrfToken();
        $this->render('admin/exams/marks-entry', ['classes' => $classes, 'csrf_token' => $csrfToken]);
    }

    public function classMarksEntry($classId) {
        $class = $this->db->selectOne("SELECT * FROM classes WHERE id = ? AND is_active = 1", [$classId]);
        if (!$class) {
            $this->session->setFlash('error', 'Class not found');
            $this->redirect('/admin/exams/marks-entry');
        }
        ...
        // Get existing exams for this class
        $exams = $this->db->select("
            SELECT * FROM exams
            WHERE class_id = ? AND is_active = 1" . ($academicYearId ? " AND academic_year_id = ?" : "") . "
            ORDER BY exam_name
        ", $academicYearId ? [$classId, $academicYearId] : [$classId]);

        $csrfToken = $this->csrfToken();
        $this->render('admin/exams/class-marks-entry', [
            'class' => $class,
            'students' => $students,
            'subjects' => $subjects,
            'exams' => $exams,
    ...
    public function admitCards() {
        ...
        // Get exams with their associated classes
        $exams = $this->db->select("
            SELECT e.*,
    ...
    public function getExamClasses($examId) {
        try {
            ...
            // If no exam_subjects found (for backward compatibility), use the exam's class_id
            if (empty($examClasses)) {
                if ($exam['class_id']) {
                    ...
                    // If no class_id, get all active classes
                    $academicYearId = $this->getCurrentAcademicYearId();
                    $where = "WHERE is_active = 1";
                    $params = [];
                    if ($academicYearId) {
                    ...
                    $examClasses = $this->db->select("SELECT id, class_name, section FROM classes $where ORDER BY class_name", $params);
                }
            }

            $this->json(['classes' => $examClasses]);
        } catch (Exception $e) {
    ...
    public function getExamStudents($examId) {
        ...
        $whereClause = "s.class_id IN ($placeholders) AND s.is_active = 1 AND s.tc_issued = 0";
        $params = $classIds;
        ...
        // Get students from all relevant classes (or filtered class)
        $students = $this->db->select("
            SELECT s.*, c.class_name, c.section
            FROM students s
            LEFT JOIN classes c ON s.class_id = c.id
