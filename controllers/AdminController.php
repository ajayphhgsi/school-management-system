...
class AdminController extends Controller {
    ...
    public function students() {
        ...
        // If filter is 'all', no where clause
    ...
    public function tcCertificates() {
        ...
        $this->render('admin/certificates/tc', ['certificates' => $certificates]);
    }
    ...
    public function generateCertificate() {
        try {
            ...
            if ($certificateType === 'transfer') {
                $pdfPath = $this->generateTransferCertificatePDF($student, $data);
            } else {
            ...
            $this->db->insert('certificates', $certData);
    ...
    private function generateTransferCertificatePDF($student, $data) {
        ...
        $pdf->SetFont('times', '', 12);
        $pdf->SetXY(25, 45);
        $pdf->MultiCell(0, 6, $schoolAddress, 0, 'C');
        ...
        $pdf->SetXY(30, $y);
        $pdf->Cell(50, 8, 'This is to certify that', 0, 0);
        ...
        $pdf->SetFont('times', 'B', 12);
        $pdf->Cell(0, 8, $admissionDate, 0, 1);
        ...
        $y += 8;
        $pdf->SetXY(30, $y);
        $pdf->Cell(50, 8, 'Academic Record:', 0, 0);
        $pdf->SetFont('times', 'B', 12);
        $pdf->Cell(0, 8, $academicRecord, 0, 1);

        $pdf->SetFont('times', '', 12);
        $y += 8;
        $pdf->SetXY(30, $y);
        $pdf->Cell(50, 8, 'Conduct:', 0, 0);
        $pdf->SetFont('times', 'B', 12);
        $pdf->Cell(0, 8, $conduct, 0, 1);

        $pdf->SetFont('times', '', 12);
        $y += 8;
        $pdf->SetXY(30, $y);
        $pdf->Cell(50, 8, 'Reason for Leaving:', 0, 0);
        $pdf->SetFont('times', 'B', 12);
        $pdf->Cell(0, 8, $reasonText, 0, 1);

        $pdf->SetFont('times', '', 12);
        $y += 8;
        $pdf->SetXY(30, $y);
        $pdf->Cell(50, 8, 'Date of Issue:', 0, 0);
        $pdf->SetFont('times', 'B', 12);
        $pdf->Cell(0, 8, $issueDate, 0, 1);
        ...
        $pdf->Line(30, $y + 15, 80, $y + 15);

        // Principal signature
        $pdf->SetXY(80, $y);
        $pdf->Cell(50, 5, 'Principal', 0, 0, 'C');
        $pdf->Line(80, $y + 15, 130, $y + 15);

        // School Seal
        $pdf->SetXY(130, $y);
        $pdf->Cell(50, 5, 'School Seal', 0, 0, 'C');
        $pdf->Line(130, $y + 15, 180, $y + 15);
    ...
    public function saveSettings() {
        ...
        $settingKeys = [
            ...
            'scholar_auto_generate', 'school_logo'
        ];
